# project_to_zip.ps1
#
# Extracts relevant files from the animo repository and packages them into a ZIP archive.
#
# Usage:
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\project_to_zip.ps1
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\project_to_zip.ps1 -SourceDir "C:\Users\hiroxpepe\Projects\animo" -OutZip "animo_export.zip"
#
# Parameters:
#   -SourceDir  : Repository root directory (default: same folder as this script)
#   -OutZip     : Output ZIP path (default: <parent>/animo_yyyyMMdd_HHmmss.zip)
...
if (-not $OutZip) {
    ...
    $OutZip = Join-Path (Split-Path $SourceDir -Parent) "animo_${ts}.zip"
}
...
$excludeDirSet = [System.Collections.Generic.HashSet[string]]::new(
    [string[]]@(
        ...
        # Package managers
        'node_modules',
        # Unity auto-generated (if animo is ever opened as a Unity project)
...
$excludePathPrefixes = @(
    # No project-specific path exclusions for animo
)
