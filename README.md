...
## How a Frame Works (`Engine.Live(delta_time)`)
...
Phase 2-4 closed every TBD left in v0.1.4. The 17 ambiguities (Q1–Q17) — `Affect` with NaN, `Live` with negative `delta_time`, what `Lock` does when called twice, threading guarantees, and so on — all have a final answer now. Decisions are logged in [`docs/decisions/v0.1.5_ambiguity_resolution.md`](docs/decisions/v0.1.5_ambiguity_resolution.md), the spec is reissued as `animo_spec_v0.1.5_EN.md` / `animo_spec_v0.1.5_JP.md`, and the schema accepts `schema_version: "1.5"` while constraining `commitment.bonus` to the new `[0, 50]` range.
...
| ---------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| ✅ true          | Agent.Update missing — NPCs would freeze after Awake                                                     | Q-S80 adds `void Update() { _engine.Live(delta_time: Time.deltaTime); }`                   |
...
| ✅ true          | ScenarioRunner `current_time += delta_time` IEEE-754 drift breaks Q-S35's "exactly floor(duration / delta_time)" promise | Q-S84 switches to integer `for (int i = 0; i < total_steps; i++)`                  |
...
| Q-S93 | spec §26.3 promised TraceResult.behavior_count / behavior_total_time / ToCsv() / ToJson() but Q-S82 shipped only agent_id/duration/delta_time/frames; analysis surface completely missing                       | Added all 4 spec-promised members as Phase 3 stubs                                                          |
...
| Q-S98  | Q-S84's `(int)Math.Floor(duration / delta_time)` does FLOAT division; `float32 (10.0f/0.1f) = 99.9999985... → Floor = 99` — Q-S35's "exactly floor(duration/delta_time)" was STILL false even after Q-S84 | Promoted to double + Math.Round: `(int)Math.Round((double)duration / (double)delta_time)` — corrects sub-LSB drift symmetrically           |
| Q-S99  | Q-S42 declared `${template_id}_run_${_sequence++}` auto-generation but Q-S82's file materialization missed the `_sequence` field — same pattern as Q-S92's `_engine` omission                       | Added `int _sequence = 0;` instance field with #pragma CS0169 suppression                                                               |
...
+ **Round 2** (deeper grep over actual code blocks): 1 contradiction caught — ScenarioRunner.cs class docstring (line 50) still wrote `Math.Floor(duration / delta_time)` after Q-S98 spec narrative was fixed; corrected to mirror the Q-S98 Math.Round form.
...
| Q-S109 | Q-S42 narrative wrote `${template_id}_run_${seq++}` but `Run(string agent_id, ...)` parameter is named `agent_id`; `template_id` not in scope → "name does not exist" compile error.                                                                                     | sed-unified narrative on `${agent_id}_run_${_sequence++}`.                                                                                                                               |
...
| Q      | Attack                                                                                                                     | Resolution                                                                                                                      |     |                                  |
| ------ | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | --- | -------------------------------- |
| Q-S114 | Q-S109's sed accidentally swept C# code blocks; `${agent_id}_run_${_sequence++}` (Bash/JS) left in C# string-interp contexts    | Restore C# form `$"{agent_id}_run_{_sequence++}"` in code blocks; narrative historical citations preserved.                          |     |                                  |
...
| Q-S117 | ScenarioRunner.Run delta_time=0 → +Infinity → (int)int.MinValue → silent empty TraceResult                                         | Run entry: `if (delta_time <= 0.0f) throw new ArgumentException(...)`.                                                                  |     |                                  |
...
| Q-S123        | ScenarioRunner.Run `current_time = total_steps * delta_time` is unused (CS0219)                                                                                    | Remove the dead line; sweep semantics unchanged.                                                                                                      |
...
| Q-S126        | §9.2 Lock(0) narrative implies "is_locked stays true until next Live(delta_time)" but test requires immediate `is_locked=False`                                    | Clarify: `is_locked => _lock_remaining > 0` makes Lock(0) immediately observable; no special path needed.                                             |
