...
**Q-S109 sed の C# string-interp 汚染 (Q-S114)。** Q-S109 の narrative sed `template_id` → `agent_id` が C# コードブロックまで巻き込み、`${agent_id}_run_${_seq++}`（Bash/JS）が C# string-interp コンテキストに残置 → Phase 3 で「予期しない '$'」CS エラー。Q-S114 でコードブロックは `$"{agent_id}*run*{_seq++}"` 復元、narrative 引用は元のまま。 (v0.1.5, Q-S114)
