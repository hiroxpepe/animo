# Animo Development Log

> A round-by-round record of the hard review this project went
> through, with Gemini, on the way from v0.1.4 to v0.1.5. Each
> entry, marked `Q-Sn`, is one a thing found, checked by search, before
> any change was made. For the settled, final design, see
> [animo_spec.md](animo_spec.md).
...
down the line. The one, real exception is **an with no end change on
...
Three questions on the Lock the steps, in order (Q-S1/S2/S3), three on the
...
OR-a flag that holds its own state within one frame; a second `Store.Register` gives a
...
own a flag that holds its own state would vanish while locked (used up by Step 4, inside the
Lock's own window, before any Step 5, once unlocked, could ever
see it); `reset_threshold`'s own default of `trigger - 5.0` could
go below zero for a low trigger, catching, with no way out the Threshold in `Above`
...
line. The fix: the a flag that holds its own state only clears while `!is_locked`; the
...
**Q-S13/S14/S15** cleaned up what was left over: Q-S10's own a flag that holds its own state,
reaching across a Lock, held up fine under a real *survival* test,
but, given the Mermaid the way things are laid out of Phase_2_4_6, it skipped the
...
a flag that holds its own state are held back while locked; only the very first frame after
unlocking uses up the a flag that holds its own state, keeping §9.7.1's own promise of
...
every time. The order sort is now **held steady**, against the
order composed `influences[]` are given in; §8.3's own merge rule
for `influences` now matches the change made to `actions`
(Persona's own order first); the LLM has exactly one thing left to set (the
...
`Update` on a another built from the same start already sent through `OnDestroy` — a real
break of how Unity's own set of steps a thing goes through should work. One line,
...
anything; a a case where two things do not match gives a Warning, and does nothing at all.
...
`_effective_needs`) switched, the right way. Threshold now reads
...
across different sources. Q-S24 builds again the graph in terms of
...
in the spec and the real contract under it. **Q-S25**: §12.3.2's
...
"one who does not fight villager" Persona leaving out `fear` would read
...
Engine's own constructor now sets aside spots `0..7` for the eight
...
right at the a place made ready for change between a promise made and the real world.
**Q-S28**: a spawned copy with a fixed `agent_id`, given straight
from JSON, along with Q-S6's own defense of keeping the first,
turned 99 of 100 spawned goblins into things cut off, with no way to send their own signal, cut off from the
...
Persona, and copies it deep on its own. A start up pattern
...
(an NPC could run out of air while still out exploring). A new, open
...
last turn could ever pick it up. The outside check becomes
...
catching a clash that came, in turn, from the fix made in the round
...
caught right at the a place made ready for change between rules Phase_2_4_12 had only just put
...
outside check is `current_time < duration` (strict, with no EPSILON),
...
into an Engine whose own, standard spot the way things are laid out (from Q-S27) put
...
give a spot to a Need known only through needs_meta → B: bake in
...
a spot to a Need known only through needs_meta, where Q-S30 and
...
the right way, but ran no `Live(delta_time)` at all, right after — so
...
check for an unused now moves to Stage 2 (where it can see the
...
gap a writer would ever leave between real marked points (5 or more,
...
English (§13.2's own Mermaid had no new point, for Stage 2's own P9f,
for A038; §13.2.1's own the comment describing it for ValidateStage2 named neither
...
no worked-out block of example, made-up code). Round 2 caught 2 more, that
...
their own, to hold clashes of scope and a slip in the mathematics, every
...
the claim that "a writer's own gap between marked points is always
5 or more, by A035 / Q-S15" — a a case where two things do not match of kind: A035's own gap of
...
build error, confirmed. Q-S48 adds the naming,
...
A038's the rule for what is unused, named `needs[]`, `actions[].need`, and
...
unused. Q-S49 adds this fourth site to the "in use" check: the true
...
the right place; `Store` stays the the list that keeps track, only for a real, live
...
§13.2.1's own the comment describing it for ValidateStage2 named neither A039 nor
...
call site for ApplyNonTierMetadata, gave no a reference at all to where
...
own EPSILON, at 0.01f; Q-S48's own naming of
...
once Engine.cs ships the naming; one each for the rest).
...
own, to hold a break of scope, a weak point in how a claim was backed,
...
tie-break was written, in the spec's own words, using a short form
...
spec's own words pointed to `actions.First(...)` is written again, away
...
(A038's own unused check, and rates): Q-S41 and Q-S49, together,
...
was wrongly marked as unused. A fifth site, `rates.keys()`,
...
setting, a old, and no longer true Agent's own reference would pile up, corrupting
...
`$"{template_id}_{GetInstanceID()}"`, holds, the right way, within one,
single Unity session, but gives no fixed, repeatable id across a
a set of machines joined together — a played over more than one machine game needs some other, fixed source for its
...
real removal, chances of harm a child quietly losing a fallback it truly
...
since v0.1.2's own move to a flat `float[]`, only ever a catch for a
...
the comment describing it for ValidateStage2 missing rates; §11.6's own note on
PersonaCache's stage 2, missing Q-S57; §3.5.2's own PHASE C, missing
a a reference to where ApplyNonTierMetadata was declared; §3.1's own
...
holding a old, and no longer true reference to `actions.First(...)`). Rounds 2-3
...
apart, as a a quote of the old, pre-Q-S52 wording, kept only as a
...
round — **the the tool that turns code into a program's own attack**. After Phase_2_4_16 closed
...
spelling wrong, or a missing naming, that would stop any Phase 3
builder, right at the C# the tool that turns code into a program, before a single test could ever
run. Logic, it turns out, does not age; the way code must be written does.
...
override, from Q-S28, would corrupt every another built from the same start drawn from it.
...
a case where two things do not match of type. Both PHASE A loops are set right, to
`_persona.needs?.values ?? new Dictionary<string, float>()`.
**Q-S66** (Q-S56 undid its own fix): Q-S56's own write again of PHASE C's
"Step 3" wrote
...
write again. Q-S66 fixes it by working straight through `_need_index`
...
conflict with itself): §16.6's own table of Engine fields declared
...
example, made-up code, for the T0 timer stage, and §24.3's own words, both
point to `_lock_remaining`, but that field had no row at all, in
§16.6's own table of Engine fields, and no naming at all in
...
spec, is properly set apart, as a a quote of the old wording, from
...
right at the C# the tool that turns code into a program. The red baseline grows from 305 to
...
nine-hit round — **the the tool that turns code into a program's own attack, a second wave**.
...
Stage 2 a thing found, per template, into the whole record kept by
...
at all. Q-S78 sets it right, by the type's own name:
...
type, at build time. Q-S79 ships the file, its own the naming of things
...
the tool that turns code into a program. The red baseline grows from 313 to **322 EditMode tests**,
...
the false ones at the very first search — the owner's own set of steps
holds.
...
| real | Store.Unregister's own shape not matching (a real `Animo.Agent`, against the interface `IAnimoAgent`) | Q-S81 sets it, throughout, to the interface's own shape |
...
| real | MockScene's own Tick sets aside a new ToArray and a new array, every frame — 432,000 costs, in one hour of a soak test | Q-S87 brings in a `List<T>`, kept and used again, as scratch space |
| real | §16.2.2.1's own Q-S27 example, made-up code runs alongside §3.5.2's own, true constructor | Q-S88 marks it plainly, as a sketch, only for the idea, with a clear a reference |
...
| Q-S93 | The spec's §26.3 promised `TraceResult.behavior_count`, `behavior_total_time`, `ToCsv()`, and `ToJson()`, but Q-S82 shipped only `agent_id`, `duration`, `delta_time`, and `frames` — the whole surface for looking things over was simply missing | Added all four, promised members, as Phase 3 stand- |
| Q-S94 | The spec's own words wrote `com.meowtoon.{animo,germio,briko,utilo}`, but the real package.json shipped `com.studiomeowtoon.*` — a UPM a thing it needs would fail to resolve | Set the spec, in both English and Japanese, to `com.studiomeowtoon.*` (the side that matches the real build, and the author's own name) |

Phase_2_4_20's own round of checking for a clash with itself:

+ **Round 1** caught one clash: §17's own Repository Layout held
  two the ways things are laid out, side by side, but only one had been set right, to
...
true, by search, **zero made up**. The set of steps crosses Q-S100 —
...
+ **Round 2** (a deeper search, through the real code): one clash
  caught — ScenarioRunner.cs's own class the comment describing it (line 50) still
...
hundredth — a hundred things found, each one checked by search, owner
against Gemini, every one folded into the spec, with not a single
made-up fix ever slipping into the all the code.
...
itself — Gemini was right to see that Q-S96 (Agent's own OnDestroy, made
...
| The Q | The attack | What was done |
| --- | --- | --- |
| Q-S101 | Q-S96's own spec did not match the real file — the agent_id reader still read `_composed_persona.agent_id` straight, and OnDestroy still called Store.Unregister, with no guard at all | Carried the two-line fix back into the real Scripts/Agent.cs, plus a note, in the class's own the comment describing it, pointing to Q-S101. **A step up, in the rule itself**: from Q-S101 on, any spec fix that touches a code block now sets off a search, through `Scripts/*.cs`, to prove the real file stays in step. |
...
a thing found after the hundredth — and it caught the very shape of bug
...
| The Q | The attack | What was done |
| --- | --- | --- |
| Q-S102 | Q-S44's own change to how an Animator's state name was expanded broke every spawn — a Unity Animator Controller uses a fixed, edit-time state name, never a name built at runtime, with GetInstanceID() folded in. Every NPC stood, frozen, in a T-pose. | A not whole undo of Q-S44: `_animator?.Play(stateName: _engine.behavior)` (the plain id); `GetExpandedActionTrigger` is kept, only for the path through the Bus. |
...
| Q-S105 | A039's own example, made-up code wrote `next.trigger - prev.trigger`, but `Threshold.trigger` is, in fact, a `string`; the real, float field is `trigger_threshold`. Copied straight, this is a build error. | Set right, by search, everywhere, to `trigger_threshold`. |
...
| Q-S107 | Step3_Thresholds read `_persona.binding.thresholds` straight, while the constructor used `?.thresholds ?? Array.Empty<...>()` — a a case where two things do not match, in how deep the defense went; a Persona, built by hand, threw a null-reference error, every single frame. | Step 3 now reads the very same, null-safe form the constructor uses. |
| Q-S108 | The schema's own `reset_threshold` held `"minimum": 0.0`; ajv turned it down, with no way past it, before the Validator's own A034 (from Q-S11) could ever show its own, readable Error. A034 had become a rule that could never, in fact, be reached. | The schema's own minimum was removed, letting a real value flow through, to A034. The upper bound, at 100.0, stays. |
...
| Q-S111 | PersonaCache.GetComposed threw one, plain InvalidOperationException, for two errors that were different in kind (Initialize never called, against a real writer's own mistake, per template); Awake's own catch named both, alike, as "Stage 2, fail loud." A missing Bootstrapper could never be told apart from the other, when something went wrong. | Two, separate kinds of exception: `PersonaCacheNotInitializedException` (a real break in the build itself; let it rise), and `PersonaTemplateRejectedException` (one writer's own mistake, per Agent; catch it, and turn that Agent off). |
| Q-S112 | §12.1 stated "a written record a Warning once, then stay quiet," for a null Bus, but Awake leaned on `_bus?.Publish(...)`, to skip in silence. A build the steps, in order that stripped the Bus out, as null, would leave every Threshold's own firing simply gone, with no word at all. | At the start of Awake: `if (_bus == null) AnimoLog.Warning(...)`, once, before anything else runs. |
| Q-S113 | A009 guarded `actions[].id` against being empty, but only ever assumed (never checked) that each was, in fact, unique. Two, doubled ids, from an LLM, would overwrite one another, with no word given, inside `_cached_action_triggers`. | A new, Stage 2 Error rule, **A040** (checking that composed actions stay unique). **The count of Validator rules: 40 → 41** (A000-A040). |
...
+ **Round 2** (the new layer, from Q-S101 — real code blocks,
  ...
  class the comment describing it. The three Q's that touch Awake's own body
...
both a fix on the set of steps's own self — Q-S114 cleans up a spillover
...
a the comment describing it's own listing, that Q-S113 had left out of date. The
set of steps now brings a ninth kind of self-made bug to light: **a gap
in the rule of checking itself** — a sweep, from an earlier round,
left something behind, that the next round then caught.

| The Q | The attack | What was done |
| --- | --- | --- |
| Q-S114 | Q-S109's own search-and-replace, by accident, swept through a block of C# too; `${agent_id}_run_${_sequence++}` (a Bash or JS shape) was left inside a place meant for C#'s own string building | Set back to the true, C# shape, `$"{agent_id}_run_{_sequence++}"`, inside every code block; a a quote, kept as a record, in the prose, stays as it was |
| Q-S115 | Agent.Update fixes its own read of time to `Time.deltaTime`, so a MockScene, under an EditMode test, freezes the time it simulates | Set down, in writing, an `ITimeProvider` a place made ready for change, for Phase 3's own use, in both §11.4.1 and Agent.cs's own the comment describing it. The v0.1.5 stand-in stays as it is (it never, in fact, runs) |
...
| Q-S117 | ScenarioRunner.Run, given `delta_time=0`, divides by it, giving positive a value with no end, then `(int)`, giving `int.MinValue` — an empty TraceResult, with no word at all of why | At the start of Run: `if (delta_time <= 0.0f) throw new ArgumentException(...)` |
...
| Q-S119 | Q-S113 added rule A040 to §13 and §17's own Layout, but missed Validator.cs's own the comment describing it for ValidateStage2, and §11.6.2's own listing | Set right, both the the comment describing it and §11.6.2, to list A040. **A step up, in the rule**: any new Validator rule now sets off a search, through every the comment describing it's own listing |
...
+ **Round 2** (the new layer — real code blocks, against every
  ...
  own XML the comment describing it, for ValidateStage2, had missed A040 —
  ...
  the comment describing it's own listing must now stay in step, whenever a new rule
  is added.
+ **Round 3** (a check of the README against the record of
  choices): two clashes caught — the README held no section for
...
**one turned down, as made-up claim #4** (the set of steps's own first
made-up claim since Phase_2_4_19, six clean rounds back). Of note:
three of the eleven taken up are, again, a fix on the set of steps's
...
| Q-S122 | A039's own example, made-up code wrote `< 1.0f`, against a test's own words, "a pair at 78 and 79 fires" (the English "within 1.0f" reads as with both ends counted) | Set the example, made-up code, the Mermaid, the §13 row, and a new note, all to `<= 1.0f` |
...
| Q-S124 | A019's own search, for a a spelling mistake, checks three sites; A038's own checks five (grown wider, by Q-S49 and Q-S57) — the two stand apart, with no real reason | Widened A019 to A038's own union: needs, actions, influences, thresholds, and rates |
...
| Q-S130 | Q-S118's own Editor-only guard, `(isEditor && !isPlaying)`, matches the very state an NUnit EditMode test runs under — the clean-up would run mid-a group of tests, running the chance of harm one test's own state bleeding into the next | Set the discipline, instead, at the test's own side (`Store.ResetForTesting()`, inside `[SetUp]`); the Bootstrapper's own guard stays right, for real, live use |
| **made up #4** | A035 Case01's own `trigger=0` is called "a state that should never run" — with the claim that Stage 1's own turn-down should keep Stage 2 from ever running at all | **Turned down**. The spec, at lines 3170-3181, plainly allows a caller to run Stage 2, even where Stage 1 already holds an error; the test's own the comment describing it states this, as a real, second layer of defense |
...
taken up this round, are a fix on the set of steps's own self. The
...
this set of steps. All three turn-downs were plain: claim #5 (a crash,
...
plainly stating the design's own what was meant, "after the t=0 event."
This round's nine, real things found sit, in the main, as promises made
ahead of Phase 3's own build: Q-S131 closes the last, able to be changed from outside
...
how exact it is, in the example, made-up code; Q-S139 removes a doubled line, left
...
of
this set of steps. All eleven turn-downs were clean: claim #8 (on
...
met by Agent.cs's own the comment describing it, already holding Q-S115's own
record; claim #10 (a a special value used only as a sign value, for a tier of 0) was met by
Data.cs's own words, "safe by its own build, and held in check by
A019 and A038"; claim #12 (on the Store, as a the one, and only, copy) was met by
...
open in the spec itself. The nine, real things found are all genuine
...
holds the job of writing down a record; Q-S145, an empty string), one gain, for
the one who writes the code (Q-S146, a property safe to read from a
a tool for finding mistakes), and one gap in the record (Q-S148, IsRegistered's own
promise).
...
itself — Gemini had been right to see that `79.3f-78.3f=1.0f`, in C#, but
...
set of steps sets right its own, first proof, which had leaned on
Python's own float64 mathematics.
...
time, in this whole set of steps, that Gemini quoted a real block of
...
inside the the comments describing them of Data.cs and Json.cs, keeping the eight,
