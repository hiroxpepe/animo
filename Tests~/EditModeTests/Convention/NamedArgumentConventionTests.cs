...
namespace Animo.Tests.EditMode.Convention {
    /// <summary>
    /// Roslyn gate: calls to our own types (Animo.*) must pass arguments by
    ...
    public class NamedArgumentConventionTests
    {
        static bool is_own(ISymbol? s)
        {
            ...
            return asm == null || asm.StartsWith("Animo") || s.Locations.Any(l => l.IsInSource);
        }
        ...
        public void OwnCalls_UseNamedArguments()
        {
            ...
            var comp = CSharpCompilation.Create("Animo.Scan", trees.Values, refs,
